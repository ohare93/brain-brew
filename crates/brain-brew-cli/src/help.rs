...
pub(crate) fn command(name: &str) -> Option<&'static str> {
    match name {
        ...
        "workbench" => Some(
            "Usage:\n  brainbrew workbench serve [--manifest brainbrew.yaml] [--port <port>] [--no-open] [--dev-assets <dir>]\n\nExamples:\n  brainbrew workbench serve --manifest brainbrew.yaml\n  brainbrew workbench serve --manifest brainbrew.yaml --port 0 --no-open\n  brainbrew workbench serve --manifest brainbrew.yaml --dev-assets build/workbench\n\nStarts a localhost Deck Workbench server bound to 127.0.0.1. Port 0 selects an available port. Release builds serve embedded UI assets; --dev-assets serves a development asset directory. The JSON API exposes health and workspace metadata for the browser UI.\n",
        ),
        "verify" => Some(
