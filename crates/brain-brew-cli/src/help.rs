...
pub(crate) fn command(name: &str) -> Option<&'static str> {
    match name {
        ...
        "import" => Some(
            "Usage:\n  brainbrew import crowdanki <deck-folder> --accept-suggested-ids [--force] --out deck.yaml\n\nExamples:\n  brainbrew import crowdanki build/de-extended --accept-suggested-ids --out deck.yaml\n  brainbrew import crowdanki build/de-extended --accept-suggested-ids --force --out deck.yaml\n\nImport creates a new canonical file by default and refuses any existing output. `--force` replaces an existing regular file through a fingerprint-checked, backed-up, recoverable workspace transaction; it never replaces a directory or symlink.\n",
        ),
        "lock" => Some(
