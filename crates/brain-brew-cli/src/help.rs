...
pub(crate) fn command(name: &str) -> Option<&'static str> {
    match name {
        ...
        "verify" => Some(
            "Usage:\n  brainbrew verify [--manifest brainbrew.yaml] (--all-targets | --target <target>) [--include package/brainbrew.yaml ...] [--package-root packages/] [--media-root media/ | --media-root <package-id>=<dir> ...] [--translation-coverage lenient|strict] [--skip-content-validation]\n\nExamples:\n  brainbrew verify --manifest brainbrew.yaml --all-targets\n  brainbrew verify --manifest brainbrew.yaml --target da-standard\n  brainbrew verify --manifest america/brainbrew.yaml --include ultimate-geography/brainbrew.yaml --target en-america\n  brainbrew verify --manifest brainbrew.yaml --all-targets --media-root media/\n  brainbrew verify --manifest brainbrew.yaml --target de-release --translation-coverage strict\n  brainbrew verify --manifest brainbrew.yaml --target legacy-sloppy-html --skip-content-validation\n\nBy default verify structurally checks rendered deck descriptions and card templates as lightweight HTML fragments, and note-type styling as balanced CSS. Use --skip-content-validation as an escape hatch for legacy Anki content that renders correctly despite a false positive.\n",
        ),
        "explain" => Some(
