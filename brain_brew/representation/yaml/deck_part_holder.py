...
class DeckPartHolder(Generic[T]):
    ...
    @classmethod
    def from_deck_part_pool(cls, name: str) -> T:
        return cls.get_file_manager().deck_part_from_pool(name)

    @classmethod
    def override_or_create(cls, name: str, save_to_file: Optional[str], deck_part: T):
        fm = cls.get_file_manager()

        dp = fm.deck_part_if_exists(name)
        if dp is None:
            dp = fm.new_deck_part(DeckPartHolder(name, save_to_file, deck_part))
        else:
