...
class FromDeckParts(DeckPartBuildTask):
    ...
    class Representation(RepresentationBase):
        ...
        note_models: Optional[List[dict]]
    ...
    def from_repr(cls, data: Union[Representation, dict]):
        ...
        return cls(
            ...
            note_models=list(map(FromDeckParts.DeckPartToRead.from_dict, rep.notes)),
            headers=list(map(FromDeckParts.DeckPartToRead.from_dict, rep.notes))
        )
    ...
    def execute(self):
        ...
        for model in self.note_models:
            DeckPartHolder.override_or_create(name=model.name, save_to_file=None, deck_part=Notes.from_file(model.file))
        for header in self.headers:
            DeckPartHolder.override_or_create(name=header.name, save_to_file=None, deck_part=Notes.from_file(header.file))
