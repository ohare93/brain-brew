...
Use the version recommended by the deck repository you are contributing to. The current Brain Brew preview is `v1.0.0-alpha.6`; pin examples and contributor docs to that immutable tag and verify the installed binary with:
...
After the alpha.6 release gates pass and the maintainer manually publishes it, install the crate from crates.io:
...
```bash
release=https://github.com/jeprecated/brain-brew/releases/download/v1.0.0-alpha.6
...
```bash
nix run github:jeprecated/brain-brew/v1.0.0-alpha.6 -- targets --manifest brainbrew.yaml
```
