...
#
#     def test_warns_of_note_model_overwrite_in_subconfig(self, csv_source_default):
#         def setup_config(x, y):
#             x.config_entry = y
#
#         with patch.object(SourceCsvCollection, "setup_config_with_subconfig_replacement", setup_config), \
#              patch.object(SourceCsvCollection, "verify_config_entry"):
#             csv_config = {
#                 BuildConfigKeys.NOTES.value: "test",
#                 BuildConfigKeys.SUBCONFIG.value: [
#                     {**csv_source_default.config_entry, **{BuildConfigKeys.NOTES.value: "notes.json"}},
#                 ]
#             }
#
#             with patch.object(logging, "warning") as mock_warning:
#                 source = SourceCsvCollection(csv_config, read_now=False)
#                 assert mock_warning.call_count == 1
#
#     def test_passes_data_to_source_csvs_correctly(self, csv_source_default):
#         def setup_config(x, y):
#             x.config_entry = y
#
#         notes = "test_notes.json"
#         with patch.object(SourceCsvCollection, "setup_config_with_subconfig_replacement", setup_config), \
#              patch.object(SourceCsvCollection, "verify_config_entry"):
#
#             source = SourceCsvCollection(
...
#             )
#
#             for csv in source.source_csvs:
#                 assert csv.notes.file_location == DeckPartNotes.formatted_file_location(notes)
#
#
