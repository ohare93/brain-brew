...
class NixE2ePartitionTests(unittest.TestCase):
    ...
    def test_browser_e2e_is_owned_by_the_prepared_devenv_job(self) -> None:
        self.assertIn("browser-e2e", self.ci)
        for workflow in [self.ci, self.release]:
            self.assertIn("devenv shell e2e", workflow)
            self.assertIn("devenv", workflow)
    ...
    def test_ci_requires_nix_package_and_prepared_browser_gates(self) -> None:
        self.assertIn("nix-package:", self.ci)
        self.assertIn("browser-e2e:", self.ci)
        self.assertIn(".#checks.x86_64-linux.brainbrew", self.ci)
        self.assertNotIn(".#checks.x86_64-linux.workbench-e2e", self.ci)
        self.assertNotIn("if: ${{ false }}", self.ci)

    def test_release_plan_waits_for_the_same_required_gates(self) -> None:
        self.assertIn("quality-gates:", self.release)
        self.assertIn(".#checks.x86_64-linux.brainbrew", self.release)
        self.assertIn("devenv shell e2e", self.release)
        self.assertIn("- quality-gates", self.release)
